...
class SFNN(nn.Module):
    ...
    def init_connectivity(self, input_size, output_size, batch):
        """
        ...
        self.init_sparsity_matrix(batch)
        self.init_hidden_state_matrix(batch)
        self.init_post_neuron_state_matrix(batch)

    def init_sparsity_matrix(self, batch):
        """
        initialize nxn adjacency matrix
        Assumes the matrix is ordered as follows [input, hidden, output]
        """
        self.Adjacency_matrix = torch.bernoulli(torch.ones(batch*self.total_neurons, batch*self.total_neurons)/2)
        self.vallidate_Adjacency_matrix(batch)

    def vallidate_Adjacency_matrix(self, batch):
        """
        ...
        self.Adjacency_matrix[:self.input_layer_size*batch, :self.input_layer_size*batch] = 0
        self.Adjacency_matrix[:self.input_layer_size*batch, -self.output_layer_size*batch:] = 0

        # hidden cant connect to input
        self.Adjacency_matrix[self.input_layer_size*batch:self.input_layer_size*batch+self.hidden_layer_size*batch, :self.input_layer_size*batch] = 0

        # output can't connect to input
        # output can't connnect to output
        self.Adjacency_matrix[-self.output_layer_size*batch:, :self.input_layer_size*batch] = 0
        self.Adjacency_matrix[-self.output_layer_size*batch:, -self.output_layer_size*batch:] = 0

        # assert at least one connection from the input layer to hidden
        if not self.Adjacency_matrix[:self.input_layer_size*batch].sum():
            self.Adjacency_matrix[0, self.input_layer_size*batch] = 1

    def init_hidden_state_matrix(self, batch):
        """
        GRU hidden states placeholder (BxNxNxD)
        """
        self.hidden_state = torch.randn(batch*self.total_neurons, batch*self.total_neurons, self.neuron_size)

    def init_post_neuron_state_matrix(self, batch):
        """
        Neuron values placeholder, after FC layer (NxD)
        """
        self.post_neuron_state = torch.randn(batch*self.total_neurons, self.neuron_size)
    ...
    def forward(self, obs: torch.tensor, reward: torch.tensor):
        ...
        placeholder = torch.zeros(batch*self.total_neurons, batch*self.total_neurons, self.neuron_size) # NxNxD
        
        #create observation tensor
        input_signal = obs.reshape(self.input_layer_size*batch, 1).expand(self.input_layer_size*batch, self.neuron_size) # NxD
        
        for tick in range(self.ticks):
            #process input FC layer
            self.post_neuron_state[:self.input_layer_size*batch] = self.input_layer_neuron(input_signal) # input_layer_size x D

            #process input synapse        
            connected_hidden_neurons = self.Adjacency_matrix[:self.input_layer_size*batch] == 1        
            placeholder[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]], \
                self.hidden_state[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]] = \
                    self.input_layer_synapse(self.post_neuron_state[:self.input_layer_size*batch].repeat_interleave(connected_hidden_neurons.sum(1), dim=0),
                                             self.post_neuron_state[connected_hidden_neurons.nonzero()[:,1]],
                                             reward.repeat_interleave(connected_hidden_neurons.reshape(batch, self.input_layer_size, -1).sum([1,2]), dim=0),
                                             self.hidden_state[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]],
                                             self.lr)
            
            #process hidden FC (sum all incoming edges then pass to FC)
            self.post_neuron_state[self.input_layer_size*batch : self.input_layer_size*batch + self.hidden_layer_size*batch] = \
                self.hidden_layer_neuron(placeholder[:, self.input_layer_size*batch : self.input_layer_size*batch + self.hidden_layer_size*batch].sum(axis=0))
            
            #process hidden synapse        
            connected_hidden_neurons = self.Adjacency_matrix[self.input_layer_size*batch : self.input_layer_size*batch + self.hidden_layer_size*batch] == 1
            placeholder[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]], \
                self.hidden_state[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]] = \
                    self.hidden_layer_synapse(self.post_neuron_state[self.input_layer_size*batch : self.input_layer_size*batch + self.hidden_layer_size*batch].repeat_interleave(connected_hidden_neurons.sum(1), dim=0),
                                             self.post_neuron_state[connected_hidden_neurons.nonzero()[:,1]],
                                             reward.repeat_interleave(connected_hidden_neurons.reshape(batch, self.hidden_layer_size, -1).sum([1,2]), dim=0),
                                             self.hidden_state[connected_hidden_neurons.nonzero()[:,0]+self.input_layer_size*batch, connected_hidden_neurons.nonzero()[:,1]],
                                             self.lr)
            
            #process output FC (sum all incoming edges then pass to FC)
            self.post_neuron_state[-self.output_layer_size*batch:] = \
                self.output_layer_neuron(placeholder[:, -self.output_layer_size*batch:].sum(axis=0))

            #process output synapse
            connected_hidden_neurons = self.Adjacency_matrix[-self.output_layer_size*batch:] == 1
            placeholder[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]], \
                self.hidden_state[connected_hidden_neurons.nonzero()[:,0], connected_hidden_neurons.nonzero()[:,1]] = \
                    self.output_layer_synapse(self.post_neuron_state[-self.output_layer_size*batch:].repeat_interleave(connected_hidden_neurons.sum(1), dim=0),
                                             self.post_neuron_state[connected_hidden_neurons.nonzero()[:,1]],
                                             reward.repeat_interleave(connected_hidden_neurons.reshape(batch, self.output_layer_size, -1).sum([1,2]), dim=0),
                                             self.hidden_state[connected_hidden_neurons.nonzero()[:,0]+self.input_layer_size*batch+self.hidden_layer_size*batch, connected_hidden_neurons.nonzero()[:,1]],
                                             self.lr)
        
        #Get first element of each action neuron, then argmax to choose the action
        return self.post_neuron_state[-self.output_layer_size*batch:, 0].view(batch, self.output_layer_size).argmax(dim=1)
